...
class EmbeddedQQManager:
    ...
    def _save_deleted_accounts(self) -> None:
        temp = self._deleted_accounts_file.with_suffix('.tmp')
        try:
            temp.write_text(json.dumps(sorted(self._deleted_bot_ids), ensure_ascii=False, indent=2), encoding='utf-8')
            os.replace(temp, self._deleted_accounts_file)
        except Exception as exc:
            log.error('保存已删除 QQ 账号列表失败: %s', exc)

    def _save_accounts(self) -> None:
        temp = self._accounts_file.with_suffix('.tmp')
        try:
            temp.write_text(json.dumps([bot.persisted() for bot in self.bots.values()], ensure_ascii=False, indent=2), encoding='utf-8')
            os.replace(temp, self._accounts_file)
        except Exception as exc:
    ...
    def set_bot_version(self, bot_id: str, version_key: str) -> EmbeddedBot:
        bot = self.bots.get(bot_id)
        if not bot:
    ...
    def set_force_quick_login(self, bot_id: str, enabled: bool) -> EmbeddedBot:
        bot = self.bots.get(bot_id)
        if not bot:
            raise ValueError('账号不存在')
        if bot.process and bot.process.returncode is None:
    ...
    def _bridge_entry(self) -> Path:
        ...
        candidates.append(self._base_dir / 'core' / 'embedded_bridge' / 'elainaqq-runtime.mjs')
        for candidate in candidates:
            if candidate.is_file():
                return candidate.resolve()
        raise FileNotFoundError('未找到 ElainaQQ QQ 运行时入口: core/embedded_bridge/elainaqq-runtime.mjs')
    ...
    async def start(self, bot_id: str) -> EmbeddedBot:
        async with self._lock:
            bot = self.bots.get(bot_id) or self.create_bot(bot_id)
            if bot.process and bot.process.returncode is None:
                return bot
    ...
    def on_onebot_disconnected(self, bot_id: str, self_id: str) -> None:
        bot = self.bots.get(bot_id)
