...
class QQManager:
    ...
    async def _save_config_async(self) -> None:
        try:
            await write_json(self.config_file, self.installed_versions)
        except Exception as exc:
            log.error('保存 QQ 版本配置失败: %s', exc)
                ...
                for url in [item for item in urls if item]:
                    ...
                    for _attempt in range(512):
                        ...
                        if expected_size and downloaded == expected_size:
                            ...
                            await asyncio.to_thread(os.replace, partial, filepath)
                            return filepath
                        if expected_size and downloaded > expected_size:
                        ...
                        try:
                            async with session.get(url, headers=headers, allow_redirects=True) as response:
                                ...
                                append = bool(downloaded and response.status == 206)
                                async for chunk in response.content.iter_chunked(1024 * 1024):
                                    await asyncio.to_thread(_write_download_chunk, partial, chunk, append)
                                    append = True
    ...
    async def _record(self, version_key: str, values: dict[str, Any]) -> None:
    ...
    async def uninstall_qq(self, version_key: str | None = None) -> dict[str, Any]:
        ...
        if managed_root.exists():
            ...
            await asyncio.to_thread(shutil.rmtree, managed_root)
