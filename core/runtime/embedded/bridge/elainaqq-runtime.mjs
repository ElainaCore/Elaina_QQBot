...
class QQInstance {
  ...
  async callExtendedOneBotAction(action, params) {
    switch (action) {
      ...
      case "nc_get_packet_status":
        return this.packetBackend?.status() || {
          enabled: false,
          available: false,
          backend: "native",
          version: this.qqInfo.version,
      ...
      case "friend_poke":
      case "group_poke":
        await this.sendPoke(params);
        return null;
      case "set_group_special_title":
        await this.setGroupSpecialTitle(params);
        return null;
  ...
  async sendNativePacket(params) {
    const { cmd, data, rsp } = normalizePacketRequest(params);
    if (!this.packetBackend?.available) {
      const reason = this.packetBackend?.status?.().reason || "原始发包后端未初始化";
  ...
  normalizeCredentialDomain(value) {
    ...
    try {
      ...
      if (!parsed.hostname || !/^[a-z0-9.-]+$/i.test(parsed.hostname)) throw new Error("invalid hostname");
  ...
  async sendPoke(params) {
    ...
    checkNativeResult(await method("OidbSvcTrpcTcp.0xED3_1", packet), "发送戳一戳失败");
    return null;
  }
  async setGroupSpecialTitle(params) {
    if (!this.packetBackend?.available) {
      const reason = this.packetBackend?.status?.().reason || "原始发包后端未初始化";
      throw new OneBotActionError("设置群头衔需要可用的原始发包后端: " + reason, 1405, "set_group_special_title");
    }
    const uid = await this.resolveUid(String(params.user_id || ""));
    const packet = buildGroupSpecialTitlePacket(
      String(params.group_id || ""),
      uid,
      String(params.special_title || ""),
    );
    const method = requireNativeMethod(this.getMsgService(), "sendSsoCmdReqByContend", "set_group_special_title");
    checkNativeResult(await method(packet.cmd, packet.data), "设置群头衔失败");
    return null;
  }
  ...
  handleSentMessageUpdates(messages) {
    ...
    if (completed.length) {
      this.incomingMessageTail = this.incomingMessageTail
        .catch(() => {})
        .then(async () => {
          for (const raw of completed) await this.handleMessage(raw, true);
        });
    }
  }
  ...
  async handleIncomingMessages(msgs) {
    ...
    for (const msg of Array.from(msgs || [])) {
      ...
      try {
        ...
        logErr(id, "[MSG] 消息转换失败:", error?.message || error);
      }
    }
